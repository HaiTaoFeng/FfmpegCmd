...
#define LOGE(...) __android_log_print(ANDROID_LOG_DEBUG, "冯海涛", __VA_ARGS__)
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    LOGE("Couldn’t open file stream %s: %d(%s)", filename, err, buf);
    if (err < 0) {
        LOGE("打开输入文件失败，错误码为:%s\n",err);
        print_error(filename, err);
        if (err == AVERROR_PROTOCOL_NOT_FOUND)
            av_log(NULL, AV_LOG_ERROR, "Did you mean file:%s?\n", filename);
        LOGE(NULL, AV_LOG_ERROR, "Did you mean file:%s?\n", filename);
        exit_program(1);
    }
